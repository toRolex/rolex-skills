...
目标不是干净的 repro，而是**更高的 repro rate**。把触发循环 100 次、并行化、增加压力、缩小时序窗口、注入 sleep。一个 50% flake 的 bug 是可调试的；1% 则不是——不断把 repro rate 提高，直到它可以调试为止。
...
## 阶段 6 — Cleanup + post-mortem（清理 + 事后分析）
...
**然后问：是什么本可以防止这个 bug？** 如果答案涉及架构变更（没有好的测试 seam、纠缠不清的调用方、隐藏的耦合），带上具体细节交接给 `/improve-codebase-architecture` skill。在修复完成**之后**再提出建议，而不是之前——你现在拥有的信息比开始时更多。
