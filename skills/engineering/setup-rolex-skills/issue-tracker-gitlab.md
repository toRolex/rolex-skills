...
- **Frontier query**：用 `glab issue list -F json` 限定到 map 的 child，丢弃任何有开放中 blocker 的项：指向开放 issue 的原生 `blocked_by` 链接（`glab api projects/:id/issues/:iid/links`）、在 `Blocked by` 行有开放 issue，或有 assignee；按 map 顺序排最前者获胜。
