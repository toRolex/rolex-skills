...
2. **没有上下文会让人惊讶** —— 未来的读者会看着代码想"他们到底为什么这样做？"
...
- **架构形态。** "我们在用 monorepo。""写模型是 event-sourced 的，读模型被投影到 Postgres。"
- **上下文之间的集成模式。** "Ordering 和 Billing 通过 domain events 通信，不是同步 HTTP。"
- **带有锁定效应的技术选择。** 数据库、message bus、auth provider、部署目标。不是每个库——只是那些换掉要花一个季度的。
- **Boundary 和范围决策。** "客户数据由 Customer 上下文拥有；其他上下文只通过 ID 引用它。" 明确的"不"和"是"同样有价值。
