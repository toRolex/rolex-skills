...
4. **跑全量测试**：合并后立即跑全量测试（如 `npm run test` / `uv run pytest`，视项目而定），贴实际输出
...
1. **给自然人读的摘要**（在 `<promise>COMPLETE</promise>` 标签**之前**输出）：合并了哪些分支、每个分支的测试结果、关闭了哪些 issue（含父 PRD）、生成了哪些 merge commit 与 summarizing commit
