...
唯一的例外是同因停滞熔断，不是重试预算：同一票以逐字相同原因连续 3 轮被 Gate 拒绝时，该票转入等待用户并保留现场，不再被下一轮批次重选；这防止“当前规格/契约下无法自动满足”的票无限空转。任何新的 Gate 接受都会清零计数，原因变化也重新计数，因此正常重试不受限，也不改变“不放弃交付”的语义。
...
交付判据与目标现场判据不同：任务分支是否可交付只由分支 commits 与“已跟踪文件是否有未提交改动”决定；未跟踪文件永远不进入 merge，因此角色留下的安装产物、校验缓存或笔记不否决交付（现场保留由用户处置），不会被自动提交。目标分支的 clean 判据仍包含未跟踪文件，用户未提交修改照旧挡住合并。
