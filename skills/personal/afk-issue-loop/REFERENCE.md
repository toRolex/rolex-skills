...
普通临时失败只结束当前尝试，不进入永久 block；准备失败释放自身锁，下次写入前重新核对现场绑定。下一批重新读 open 票并按当前依赖、优先级选择，包括失败票；用同名任务分支及现场进度继续。每次由新派角色读当前代码和 Ticket 开工，不恢复旧 agent 或调度记录。
