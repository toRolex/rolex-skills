...
Reviewer 在同一现场直接修复、测试并提交，不与 Implementer 往返；已有代码合格可无新增 commit。实现/审查失败或无可交付成果均使本票本批 settled；settled 表示管线结束，包含失败，不等于通过。

全批 settled 是 barrier。只有独立审查通过的可交付分支进入成功集；有成功分支、未完成目标合并或待关闭票时，启动唯一 Merger，否则免去 Merger。Merger 顺序合并、逐分支验证修复，全部可处理分支完成且目标验证通过后，创建本批一个中文 Conventional Commit summary（允许空提交），随后逐一关闭对应 Tickets，才返回。

批末刷新固定范围内 open Tickets 及依赖，继续下一批，无总批次或失败重试次数上限。全部给定 Tickets 关闭才算完成；只剩权限、范围外前置、不可用现场等用户待办时，记录具体阻碍并结束自动推进，不冒充交付。
