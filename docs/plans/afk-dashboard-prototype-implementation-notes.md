...
- 验收：`scripts/afk-dashboard.test.mjs`（10 个黑盒 case）与既有 `afk-recovery.test.mjs`（17 个）共用公开 CLI seam，共 27 个测试通过。测试通过 `AFK_DASHBOARD_OPEN=0` 与 PATH 首位 fake opener 确保不打开真实浏览器。
