...
- 验收：`scripts/afk-dashboard.test.mjs`（21 个黑盒 case）与既有 `afk-recovery.test.mjs`（17 个）共用公开 CLI seam，共 38 个测试通过。测试通过 `AFK_DASHBOARD_OPEN=0` 与 PATH 首位 fake opener 确保不打开真实浏览器。B2 信息架构经生产 worker 页面在 1440 桌面视口验收：矩阵网格成立（表头 + 每票一行三列 cell + 可点 Agent 卡片 + 阶段空态）、无页面级横向溢出、Ticket 与 Agent 选择得到不同 Inspector、方向键与选择后焦点保持、`aria-pressed` 表达选中态、payload 与未识别事件原样渲染。移动端尺寸不作为验收条件（用户只在桌面打开页面）。
